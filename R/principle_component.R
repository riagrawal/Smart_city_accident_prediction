...
pcr_model_lat <- pcr(Latitude ~ Intersection+Light_Cond+Weather+Road_Surface+Collision_Type+Gender+Violation+Timestamp+Vehicle.Type, data=train, validation="CV")
pcr_model_long <- pcr(Longitude ~ Intersection+Light_Cond+Weather+Road_Surface+Collision_Type+Gender+Violation+Timestamp+Vehicle.Type, data=train, validation="CV")
...
# make predictions
lat_predictions <- predict(pcr_model_lat,test , ncomp=6)
long_predictions <- predict(pcr_model_long, test, ncomp=6)

#calculate rmse
pcr_rmse_latitude <- sqrt(mean((test$Latitude - lat_predictions)^2,na.rm = TRUE))
##rmse_latitude -  0.04184836
pcr_rmse_longitude <- sqrt(mean((test$Longitude - long_predictions)^2,na.rm = TRUE))  
...
return (c(lat_predictions[nrow(lat_predictions)],long_predictions[nrow(lat_predictions)],pcr_rmse_latitude,pcr_rmse_longitude))
